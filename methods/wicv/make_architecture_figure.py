...
IMPORTANT -- which variant to draw:

  --variant v1  (default)  The configuration every reported number comes from:
                shared backbone, standard BNNeck, and the three training-time
                objectives CV-Tri, CVPA and FCA on top of the identity loss.
                This is what the paper describes and ablates.

  --variant v2  Adds the two structural modules (CAN, CVT). These exist in the
                code but have NOT been trained on real data, so v2 must not be
                drawn in a manuscript whose results and ablation come from v1 --
                a figure showing components with no experimental backing is
                exactly what a reviewer will catch.

Two panels either way, because the split between what runs during training and
what survives into inference is the thing the figure has to communicate.
...
BEFORE = (62, 126, 184)
AFTER = (220, 132, 59)
NEW_FILL = (232, 241, 249)      # v2 modules only
NEW_EDGE = (36, 95, 145)
BASE_FILL = (245, 246, 248)
...
def box(
    ...
    if dashed:  # training-only blocks
...
def draw_training_panel(draw, top: float, fonts: dict, v2: bool) -> None:
    y_mid = top + 150
    centered_text(draw, 68, top + 8, "(a) Training", fonts["panel"], INK)

    # --- input: view-balanced PK batch --------------------------------------
    box(draw, 24, y_mid - 62, 118, 46, ["before view", "(front)"], [fonts["b"], fonts["s"]],
        fill=(238, 245, 251), edge=BEFORE)
    box(draw, 24, y_mid + 16, 118, 46, ["after view", "(rear)"], [fonts["b"], fonts["s"]],
    ...
    # --- shared backbone -----------------------------------------------------
    box(draw, 178, y_mid - 40, 104, 80, ["Backbone", "(shared)"], [fonts["b"], fonts["s"]])
    ...
    arrow(draw, branch_x, top + 96, 476, top + 96)
    box(draw, 476, top + 78, 68, 36, ["memory"], [fonts["s"]], fill=BASE_FILL, edge=MUTED, dashed=True)
    arrow(draw, 544, top + 96, 560, top + 96)
    box(draw, 560, top + 78, 128, 36, ["CVPA loss"], [fonts["b"]], fill=LOSS_FILL, edge=LOSS_EDGE)

    # --- branches from the neck output z -------------------------------------
    rail_x = 452
    module_x = 476
    ...
    fca_label = "FCA loss (v1)" if v2 else "FCA loss"
    box(draw, 560, y_mid + 44, 128, 36, [fca_label], [fonts["b"]], fill=LOSS_FILL, edge=LOSS_EDGE)
...
def draw_inference_panel(draw, top: float, fonts: dict, v2: bool) -> None:
    y_q = top + 44
    y_g = top + 116
    centered_text(draw, 74, top + 4, "(b) Inference", fonts["panel"], INK)

    box(draw, 24, y_q - 20, 118, 40, ["query (after)"], [fonts["b"]], fill=(253, 243, 234), edge=AFTER)
    box(draw, 24, y_g - 20, 118, 40, ["gallery (before)"], [fonts["b"]], fill=(238, 245, 251), edge=BEFORE)

    neck_label = "+ CAN" if v2 else "+ BNNeck"
    box(draw, 178, y_q - 46, 104, 132, ["Backbone", neck_label], [fonts["b"], fonts["s"]])
    ...
    if v2:
        ...
        draw.line((s(282), s(y_g), s(447), s(y_g)), fill=MUTED, width=s(2))
        draw.line((s(447), s(y_g), s(447), s(y_q + 60)), fill=MUTED, width=s(2))
        arrow(draw, 447, y_q + 60, 470, y_q + 60)
        centered_text(draw, 350, y_g - 14, "g", fonts["b"], ACCENT)
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        default="v1",
        choices=["v1", "v2"],
        help="v1 = the configuration all reported results come from (use this "
        "for the manuscript). v2 = adds CAN and CVT, which are implemented but "
    ...
    v2 = args.variant == "v2"
    width = 760
    height = 620 if v2 else 560
    split_y = 392 if v2 else 348
    ...
    title = "WICV-Net" if not v2 else "WICV-Net v2 (not yet trained -- do not use in the manuscript)"
    subtitle = (
        "Dashed blocks exist only during training. The backbone is used unmodified."
        if not v2
        else "Shaded blocks are the untrained v2 modules; dashed blocks exist only during training."
    )
    draw.text((s(24), s(18)), title, fill=INK, font=fonts["title"])
    draw.text((s(24), s(40)), subtitle, fill=MUTED, font=fonts["s"])

    draw_training_panel(draw, 66, fonts, v2)
    draw.line((s(24), s(split_y), s(width - 24), s(split_y)), fill=GRID, width=s(1))
    draw_inference_panel(draw, split_y + 16, fonts, v2)

    output_root = Path(args.output_root)
    output_root.mkdir(parents=True, exist_ok=True)
    name = "figure_09_architecture.png" if not v2 else "figure_09_architecture_v2.png"
    path = output_root / name
    image.save(path, dpi=(300, 300))
    print(f"Saved {path} ({image.width}x{image.height}px, variant={args.variant})")
    return 0
